...
class MyWidget(QMainWindow, Ui_MainWindow):
    ...
    def launch_multi(self):
        ...
        try:
            ...
            self.hide()
            os.system(
                f"cd COVIDcover && \"{sys.executable}\" \"{os.path.join(os.path.abspath(os.path.dirname(__file__)), 'COVIDcover', 'multi_build/multi_main.py')}\" {data['ip']} {data['port']} {self.user['token']} {self.user['username']}")
            self.show()
